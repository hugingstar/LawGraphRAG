...
def request_page(
    ...
    # 사업장/부서/이름/직급/연락처는 가입 시 등록된 프로필에서 가져와 보여주기만 한다.
    ...
    return templates.TemplateResponse(
        "request.html",
        {
            ...
            "profile_complete": bool(profile.department_id and profile.site_id),
            "wide": True,
        },
    )
...
def create_incident(
    ...
    # 사업장/부서/작성자 정보는 가입 시 등록된 프로필에서 가져온다.
    # 클라이언트가 보낸 값을 신뢰하지 않으므로 다른 부서를 사칭한 접수가 불가능하다.
    profile = session.get(User, user.id)
    if not profile.department_id or not profile.site_id:
        raise HTTPException(
            status_code=400,
            detail="계정에 소속 부서와 사업장이 등록되어 있지 않습니다. 관리자에게 프로필 등록을 요청하세요.",
        )
    ...
    incident.events.append(
        IncidentEvent(
            status="review_requested",
            note="심층 검토 요청 접수 (자동 조문 분석 실패 — 안전부서 수동 검토 필요)"
            if analysis_failed
            else "심층 검토 요청 접수",
...
def add_comment(
    ...
    if kind in manager_only and not user.is_manager:
        raise HTTPException(status_code=403, detail="안전부서 관리자만 사용할 수 있습니다.")
