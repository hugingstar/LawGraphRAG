...
단순 조문 검색 도구를 넘어, 현장 부서가 사고를 **심층 검토 요청**으로 접수하고 안전부서가 이를
검토·보완요청·최종결론까지 처리하는 사내 워크플로우 애플리케이션으로 확장되어 있다. 로그인/회원가입,
역할 기반 접근 제어(신청자/안전부서 관리자), 사건 상태 이력 추적, 첨부파일, 통계 대시보드를 포함한다.

## 아키텍처

```mermaid
flowchart TD
    User(["사용자 (신청자 / 안전부서 관리자)"]) -->|로그인 세션 쿠키| Auth["인증 미들웨어 (app/auth.py)"]
...
- **인증 (`app/auth.py`, `app/routers/auth.py`)**:
  ...
  - 회원가입 시 역할(신청자/안전부서 관리자), 소속 부서, 사업장, 직급, 연락처를 등록하며, 관리자 전용 화면
    (`/dashboard`, `/review`)은 신청자 계정으로 접근 시 403으로 막힙니다.

- **심층 검토 요청 (`app/routers/incidents.py`, `/request`)**:
  - 신청자가 사고일시·사고장소·사고경위·당시상황·조치내용·피해상황 6개 항목과 증빙 파일(PDF/문서/이미지,
    최대 5개·20MB)을 입력해 접수합니다. 작성자 인적사항·부서·사업장은 클라이언트 입력을 신뢰하지 않고
    로그인한 계정의 프로필 값을 그대로 사용합니다.
...
- **관리자 검토 (`/review`) / 통계 대시보드 (`/dashboard`)**: 관리자는 사업장·부서·상태·키워드로 전체
  사건을 검색하고, 각 사건의 처리 이력과 스레드를 확인하며 조문 인용을 직접 추가/제거할 수 있습니다.
  대시보드는 기간별 상태 분포와 사업장×부서 교차 집계(히트맵)를 제공합니다.

- **설정 (`/settings`)**: 로그인한 사용자가 표시 이름·직급·연락처·소속·비밀번호를 직접 수정합니다.
...
#### 4-3. 사건 관리 워크플로우 도메인 (사용자 + 심층 검토 요청)
...
classDiagram
    ...
    departments "1" *-- "*" users : 소속
    sites "1" *-- "*" users : 근무
    users "1" *-- "*" user_sessions : 로그인 세션
    departments "1" *-- "*" incidents : 요청 부서
    sites "1" *-- "*" incidents : 요청 사업장
...
**테이블 세부 설명:**
- `departments` / `sites`: 사내 부서·사업장 기준 데이터. 회원가입 시 선택하며 사건 필터/통계 집계 기준이 됩니다.
- `users`: 계정 정보. `role`이 `requester`(신청자)면 자신이 올린 요청만, `manager`(안전부서 관리자)면 전체 사건을 볼 수 있습니다.
- `user_sessions`: 서버 측 세션 저장소. 쿠키에는 토큰만 담고 실제 유효성은 이 테이블과 대조합니다.
- `incidents`: 심층 검토 요청 1건 = 1행. 작성 시 입력한 6개 항목(`occurred_at`~`damage`)과, 이를 합쳐 만든 분석 원문 `statement`, 자동/수동으로 편집되는 `citations`(적용 조문 JSON), 현재 상태 `status`를 가집니다. `created_by_user_id`(작성자)와 `assigned_manager_id`(담당 관리자)를 함께 추적합니다.
...
| `/review` | 관리자 | 전체 사건 검토 — 검색/필터, 코멘트·보완요청·최종결과 등록, 조문 마커 편집, 처리 이력 |
| `/dashboard` | 관리자 | 기간별 상태 통계, 사업장×부서 히트맵 |
...
# (Gemini API를 조문 수만큼 호출하므로 시간이 걸리고, 무료 티어는 쿼터 제한에 걸릴 수 있습니다.
#  MERGE 기반이라 중간에 멈춰도 다시 실행하면 이미 처리된 조문은 건너뜁니다.)
